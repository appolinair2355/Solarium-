...
function injectBilan(template, stats) {
  ...
  return template
    ...
    // Remplace TOUTE ligne "📈 Bilan : ..." quelle que soit la valeur déjà présente
    .replace(/📈 Bilan\s*:[^\n]*/g, `📈 Bilan : ${bilanStr}`)
...
async function autoUpdateStrategyAnnouncement() {
  ...
  try {
    ...
    for (const s of strats) {
      ...
      // Remplace TOUTE ligne "💰 Prix : ..." par le prix configuré (sans "/mois" ni texte périmé)
      const price = getStrategyPrice(promo);
      text = text.replace(/💰 Prix\s*:[^\n]*/g, `💰 Prix : <b>${price}$</b> pour avoir votre fichier qui prédit dans vos canaux — déployable sur n'importe quelle plateforme`);
