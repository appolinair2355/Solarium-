...
export default function Shop() {
  ...
  useEffect(() => {
    setLoading(true);
    const tasks = [loadCatalog(), loadIdeas()];
    if (user) tasks.push(loadPurchases(), loadIdeaPurchases());
    Promise.all(tasks).finally(() => setLoading(false));
  }, [user, loadCatalog, loadPurchases, loadIdeas, loadIdeaPurchases]);
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0b0b14, #0f172a)', padding: '0 0 60px' }}>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(100,116,139,0.2)', background: 'rgba(15,23,42,0.7)', padding: '0 24px' }}>
        {[
          ...
          { id: 'soutien',    label: '💝 Soutien', badge: null },
        ].map(t => (
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
          ...
          <div>
            {!user ? (
              <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8' }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, color: '#e2e8f0', marginBottom: 8 }}>Connexion requise</div>
                <div style={{ fontSize: 14, marginBottom: 24, color: '#64748b' }}>Connectez-vous pour voir vos achats et téléchargements</div>
                <a href="/connexion" style={{ padding: '12px 28px', borderRadius: 12, fontWeight: 800, fontSize: 14, cursor: 'pointer', background: 'linear-gradient(135deg,#fbbf24,#f59e0b)', border: 'none', color: '#1a1a1a', textDecoration: 'none', display: 'inline-block' }}>
                  Se connecter
                </a>
              </div>
            ) : purchases.length === 0 ? (
        ...
        {tab === 'soutien' && (
          <div>
            <div style={{ background: 'linear-gradient(135deg, rgba(236,72,153,0.10), rgba(168,85,247,0.06))', border: '1px solid rgba(236,72,153,0.25)', borderRadius: 14, padding: '20px 24px', marginBottom: 32, display: 'flex', gap: 16, alignItems: 'flex-start' }}>
              <span style={{ fontSize: 28, lineHeight: 1 }}>💝</span>
              <div>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#f472b6', marginBottom: 6 }}>Soutenir le projet</div>
                <div style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.8 }}>
                  Baccarat Pro est développé avec passion. Si vous souhaitez soutenir l'équipe,
                  choisissez un montant ci-dessous. Chaque soutien aide à améliorer le produit, ajouter de nouvelles stratégies et maintenir les serveurs actifs.
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
              {[
                { amount: '200', label: '☕ Café', sub: 'Un petit geste' },
                { amount: '500', label: '🍕 Pizza', sub: 'Merci !' },
                { amount: '1 000', label: '🎮 Joueur', sub: 'Supporter actif' },
                { amount: '2 000', label: '⭐ Supporter', sub: 'Super ! Merci' },
                { amount: '5 000', label: '💪 Solide', sub: 'Vraiment généreux' },
                { amount: '10 000', label: '🔥 Motivé', sub: 'Respect 🙏' },
                { amount: '25 000', label: '💎 Premium', sub: 'Incroyable !' },
                { amount: '50 000', label: '🥇 Champion', sub: 'Tu es un héros' },
                { amount: '100 000', label: '🚀 Pro', sub: 'Légende 🏅' },
                { amount: '200 000', label: '👑 VIP', sub: 'Merci infiniment' },
                { amount: '500 000', label: '💠 Elite', sub: 'Elite du projet' },
                { amount: '1 000 000', label: '🏆 Ultimate', sub: 'Parrain du projet' },
              ].map(tier => (
                <button
                  key={tier.amount}
                  onClick={() => window.open('https://paiement-s-curis.onrender.com', '_blank')}
                  style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(236,72,153,0.65)'; e.currentTarget.style.boxShadow = '0 0 18px rgba(236,72,153,0.18)'; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(236,72,153,0.22)'; e.currentTarget.style.boxShadow = 'none'; }}
                >
                  <span style={{ fontSize: 28 }}>{tier.label.split(' ')[0]}</span>
                  <span style={{ fontSize: 14, fontWeight: 800, color: '#f472b6' }}>{tier.label.slice(tier.label.indexOf(' ') + 1)}</span>
                  <span style={{ fontSize: 18, fontWeight: 900, color: '#fbbf24', letterSpacing: -0.5 }}>{tier.amount} <span style={{ fontSize: 11, fontWeight: 600, color: '#94a3b8' }}>FCFA</span></span>
                  <span style={{ fontSize: 11, color: '#64748b' }}>{tier.sub}</span>
                  <span style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: '#f472b6', background: 'rgba(236,72,153,0.12)', border: '1px solid rgba(236,72,153,0.3)', borderRadius: 8, padding: '5px 14px' }}>
                    💝 Soutenir
                  </span>
                </button>
              ))}
            ...
            <div style={{ marginTop: 32, padding: '16px 20px', background: 'rgba(236,72,153,0.06)', border: '1px solid rgba(236,72,153,0.18)', borderRadius: 12, fontSize: 12, color: '#94a3b8', lineHeight: 1.8, textAlign: 'center' }}>
              Paiement sécurisé via{' '}
              <a href="https://paiement-s-curis.onrender.com" target="_blank" rel="noopener noreferrer" style={{ color: '#f472b6', fontWeight: 700, textDecoration: 'none' }}>
                paiement-s-curis.onrender.com
              </a>
              {' '}— Mobile Money, Wave, OM et autres méthodes locales acceptés.
            </div>
          </div>
        )}
