...
export default function Comptages() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      catch { throw new Error('Réponse inattendue du serveur'); }
    ...
    <div style={{ minHeight: '100vh', background: '#0a0e1a', color: '#e2e8f0' }}>
      <nav className="navbar">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Link to="/choisir" className="btn btn-ghost btn-sm">← Retour</Link>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 16px' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <h1 style={{ margin: 0, fontSize: 20, color: '#e2e8f0' }}>Comptages — écarts entre catégories</h1>
              <p style={{ margin: '4px 0 0', fontSize: 13, color: '#94a3b8' }}>
                Suivi temps réel des séries d'absences (costumes, victoire, parité, série de cartes,
                nombre de cartes, points). Mise à jour automatique toutes les 15 secondes.
              </p>
            </div>
          </div>
          ...
          <div style={{ marginTop: 6, fontSize: 11, color: '#64748b' }}>
            <b>actuel</b> = série en cours sans apparition •{' '}
            <b style={{ color: '#fbbf24' }}>période</b> = pic de la dernière heure •{' '}
            <b style={{ color: '#a78bfa' }}>max</b> = record absolu •{' '}
            <span style={{ color: '#22c55e' }}>📈</span> = nouveau record vs bilan précédent
          </div>
        </div>
        ...
        {!loading && grouped.length === 0 && (
          <div style={{ color: '#64748b', padding: 12 }}>
            Aucune donnée disponible (le moteur n'a pas encore traité de jeu).
          </div>
        )}
