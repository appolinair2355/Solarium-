...
async function _sendWelcomeTelegram(bot_token, channel_id, stratName) {
  try {
    const msg = `🎯 Bienvenue sur le site de prédiction automatique de Sossou Kouamé !\n\n🤖 Ce canal est désormais connecté à la stratégie *${stratName || 'Prédiction Baccarat'}*.\n\n✅ Les prédictions seront envoyées automatiquement dans ce canal.\n\n📊 Restez attentifs aux signaux — chaque prédiction est générée en temps réel.`;
...
router.post('/strategies', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const predDurationMinutes = Math.max(0, parseInt(req.body.pred_duration_minutes) || 0);
    const strat  = {
      id: nextId,
      name: name.trim().slice(0, 40),
      strategy_type: isComb ? 'combinaison' : 'simple',
      pred_duration_minutes: predDurationMinutes,
      pred_duration_started_at: predDurationMinutes > 0 ? new Date().toISOString() : null,
      ...(isComb
    ...
    // Envoyer message de bienvenue Telegram pour chaque nouvelle cible configurée
    for (const t of tg_targets) {
      if (t.bot_token && t.channel_id) {
        _sendWelcomeTelegram(t.bot_token, t.channel_id, strat.name).catch(() => {});
      }
    }
...
router.put('/strategies/:id', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const predDurationMinutes = Math.max(0, parseInt(req.body.pred_duration_minutes) || 0);
    const prevStrat = list[idx];
    const prevDuration = prevStrat.pred_duration_minutes ?? 0;
    const durationChanged = predDurationMinutes !== prevDuration;
    list[idx] = {
      ...list[idx],
      name: name.trim().slice(0, 40),
      strategy_type: isComb ? 'combinaison' : 'simple',
      pred_duration_minutes: predDurationMinutes,
      pred_duration_started_at: predDurationMinutes > 0
        ? (durationChanged ? new Date().toISOString() : (prevStrat.pred_duration_started_at || new Date().toISOString()))
        : null,
      ...(isComb
        ? { multi_source_ids: (Array.isArray(multi_source_ids) ? multi_source_ids : []).map(String),
            multi_require:    multi_require || 'any',
    ...
    require('./engine').reloadCustomStrategies(list);
    renderSync.syncStrategies().catch(() => {});
    // Envoyer message de bienvenue pour les nouveaux canaux configurés
    const prevTargets = prevStrat.tg_targets || [];
    const prevIds = new Set(prevTargets.map(t => t.channel_id).filter(Boolean));
    for (const t of tg_targets) {
      if (t.bot_token && t.channel_id && !prevIds.has(t.channel_id)) {
        _sendWelcomeTelegram(t.bot_token, t.channel_id, list[idx].name).catch(() => {});
      }
    }
